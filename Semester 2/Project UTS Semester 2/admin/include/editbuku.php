...
<html>
      ...
      <section class="content">
        ...
        <div class="card card-info">
          ...
          <form class="form-horizontal" action="index.php?include=konfirmasieditbuku" method="post" enctype="multipart/form-data">
            <div class="card-body">
                ...
                <div class="col-sm-7">
                  <select class="form-control" id="kategori" name="kategori_buku">
                    ...
                    while ($data_k = mysqli_fetch_row($query_k)) {
                      ...
                      $kat = $data_k[1];
                    ?>
                      <option value="<?= $id_kat; ?>" <?php if ($id_kat == $id_kategori_buku) { ?> selected <?php } ?>><?php echo $kat; ?></option>
                    <?php } ?>
                  </select>
                </div>
              </div>
                ...
                <div class="col-sm-7">
                  <select class="form-control" id="penerbit" name="penerbit">
                    ...
                    while ($data_t = mysqli_fetch_row($query_t)) {
                      ...
                      $terbit = $data_t[1];
                    ?>
                      <option value="<?= $id_terbit; ?>" <?php if ($id_terbit == $id_penerbit) { ?> selected <?php } ?>><?php echo $terbit; ?></option>
                    <?php } ?>
                  </select>
                </div>
              ...
              <div class="form-group row">
                ...
                <div class="col-sm-7">
                  ...
                  while ($data_g = mysqli_fetch_row($query_g)) {
                    ...
                    $tg = $data_g[1];
                  ?>
                    <input type="checkbox" name="tag[]" value="<?= $id_tg; ?>" <?php if (in_array($id_tg, $array_tag)) { ?>checked="checked" <?php } ?>>
                    <?= $tg; ?> </br>
                  <?php } ?>
                </div>
              </div>
    ...
    <div class="card-footer">
      <div class="col-sm-12">
        <button type="submit" class="btn btn-info float-right"><i class="fasfa-save"></i> Simpan</button>
      </div>
    </div>
    <!-- /.card-footer -->
    </form>
